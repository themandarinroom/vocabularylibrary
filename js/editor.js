...
import { deleteGeneratedVocabularyImage, generateVocabularyImage, generateVocabularySetCover, uploadApprovedVocabularyImage } from "./vocabulary-image-cloud.js?v=collective-images-1";
...
const newItem = (chinese = "", pinyin = "", english = "") => ({ id: `${slug(english || pinyin || "item")}-${Date.now().toString(36)}`, chinese, pinyin, english, image: null, imageType: "auto", notes: "", type: "word", audio: { aiEnabled: true, teacherAudioUrl: null }, handwriting: { enabled: false, characters: Array.from(chinese.replace(/[\s？。！?!.]/g, "")) } });
...
async function requestImageCandidate(item, replaceExisting = false, alreadyConfirmed = false) {
  ...
  try {
    const result = await generateVocabularyImage(existing.id, item.id, { replaceExisting: replacingSavedImage, english: item.english, chinese: item.chinese, imageType: item.imageType || "auto" });
